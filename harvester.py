...
def infer_category(domain, title, feed_cat):
    """Determine article category from feed source, domain hints, or keywords."""
    # 1. Feed category is always trusted — every FEED_SOURCES entry is explicitly categorised
    if feed_cat in ('india', 'world', 'business', 'sports', 'tech', 'entertainment'):
        return feed_cat
    # 2. Domain override — only reached for legacy/uncategorised feeds
...
def fetch_feed(source_name, url, feed_cat, cutoff_time, seen_urls):
    ...
    try:
        ...
        added = 0
        for item in items:
            # Use is-None checks — ET elements with no children are falsy, so `or` is unsafe here
            ...
            if pub_el is None:
                pub_el = item.find(f'{{{ATOM}}}published')
            if pub_el is None:
                pub_el = item.find(f'{{{ATOM}}}updated')
            if is_atom:
                link_el = item.find(f'{{{ATOM}}}link[@rel="alternate"]')
                if link_el is None:
...
# ── Regional language feeds (Google News RSS in native language) ──────────────
REGIONAL_LANG_FEEDS = {
...
def fetch_one_regional_feed(lang, url, _retry=True):
    """Fetch Google News RSS for one language, return list of article dicts.
    Retries once after a short delay — Google News occasionally rate-limits
    back-to-back requests from the same GitHub Actions runner IP."""
    try:
        resp = requests.get(url, headers=RSS_HEADERS, timeout=14)
        if resp.status_code != 200:
            if _retry and resp.status_code in (429, 503):
                time.sleep(3)
                return fetch_one_regional_feed(lang, url, _retry=False)
            print(f'  SKIP [{lang}] HTTP {resp.status_code}')
            return []
        root = ET.fromstring(resp.content)
        articles = []
        for item in root.findall('.//item')[:25]:
            ...
            desc = re.sub(r'<[^>]+>', ' ', raw_desc).strip()[:220]

            if title and link:
                articles.append({'title': title, 'link': link, 'pub': pub, 'source': source, 'desc': desc})

        print(f'  OK  [{lang}] {len(articles)} articles')
        return articles
    except ET.ParseError as e:
        print(f'  ERR [{lang}] XML — {e}')
    except requests.RequestException as e:
        if _retry:
            time.sleep(3)
            return fetch_one_regional_feed(lang, url, _retry=False)
        print(f'  ERR [{lang}] Network — {e}')
    except Exception as e:
        print(f'  ERR [{lang}] {e}')
    return []


def harvest_regional_feeds():
    """Fetch native-language news for all 10 Indian languages and push to KV
    in ONE combined write. Cloudflare's free KV plan caps writes at 1,000/day —
    pushing 10 separate keys every 15-minute cycle (~960 writes/day on its own)
    blew through that budget by mid-day, after which every KV write silently
    failed until the next UTC day, leaving the regional panel blank for hours.
    The Worker merges this payload into its existing cache, so a language that
    fails to fetch this cycle keeps its last-known-good articles instead of
    vanishing."""
    print('\n=== REGIONAL LANGUAGE FEEDS ===')
    combined = {}
    for lang, url in REGIONAL_LANG_FEEDS.items():
        articles = fetch_one_regional_feed(lang, url)
        time.sleep(1.5)
        if articles:
            combined[lang] = {
                'lang':     lang,
    ...
    if not combined:
        print('  No regional feeds fetched this cycle — KV push skipped')
        return
    payload = json.dumps(combined, ensure_ascii=False)
    push_to_worker('/api/update-regional-feed', payload, f'regional feeds ({len(combined)}/10 langs)')
...
if __name__ == '__main__':
    ...
    # ── Part 2: Regional language feeds ──────────────────────────────────────
    print('\n[2/3] Harvesting regional language feeds...')
    harvest_regional_feeds()

    # ── Part 3: Hashtag intelligence tree (original) ──────────────────────────
    print('\n[3/3] Harvesting hashtag intelligence tree...')
